...
static void get_key(lcb_packet_t packet, const char **key, lcb_uint16_t *nkey)
{
                                   ...
                                   lcb_server_t exc)
{
    ...
    for (ii = 0; ii < instance->nservers; ++ii) {
        ...
        if (server != exc) {
            ...
            if (pkt->opcode == opcode && pkt->opaque == opaque) {
                return (int)ii;
            }
        }
    }
                                  ...
                                  protocol_binary_response_header *res)
{
    ...
    if (rc == LCB_SUCCESS) {
        ...
        TRACE_GET_END(res->response.opaque, packet->vbucket,
                      res->response.opcode, rc, &resp);
        root->callbacks.get(root, packet->cookie, rc, &resp);
    } else {
        lcb_get_resp_t resp;
                                         ...
                                         protocol_binary_response_header *res)
{
    ...
    if (key == NULL) {
        ...
        if (packet->replica < root->nreplicas) {
            /* try next replica */
            ...
            /* FIXME record trace point */
            /* FIXME remove the packet from original log */
            lcb_packet_retry(server, packet);
                                    ...
                                    protocol_binary_response_header *res)
{
    ...
    TRACE_REMOVE_END(res->response.opaque, packet->vbucket,
                     res->response.opcode, rc, &resp);
    root->callbacks.remove(root, packet->cookie, rc, &resp);
}
                                     ...
                                     protocol_binary_response_header *res)
{
    ...
    while (ptr < end) {
        ...
        setup_lcb_observe_resp_t(&resp, key, nkey, cas, obs,
                                 ...
                                 ttp, ttr);
        TRACE_OBSERVE_PROGRESS(res->response.opaque, packet->vbucket,
                               res->response.opcode, rc, &resp);
        root->callbacks.observe(root, packet->cookie, rc, &resp);
    }
    ...
    if (lcb_lookup_server_with_command(root, CMD_OBSERVE,
        ...
        lcb_observe_resp_t resp;
        memset(&resp, 0, sizeof(resp));
        TRACE_OBSERVE_END(res->response.opaque, packet->vbucket,
                          res->response.opcode, rc);
        root->callbacks.observe(root, packet->cookie, LCB_SUCCESS, &resp);
    }
}
                                   ...
                                   protocol_binary_response_header *res)
{
    ...
    TRACE_STORE_END(res->response.opaque, packet->vbucket,
                    res->response.opcode, rc, &resp);
    root->callbacks.store(root, packet->cookie, op, rc, &resp);
}
                                        ...
                                        protocol_binary_response_header *res)
{
    ...
    TRACE_ARITHMETIC_END(res->response.opaque, packet->vbucket,
                         res->response.opcode, rc, &resp);
    root->callbacks.arithmetic(root, packet->cookie, rc, &resp);
}
    ...
    if (status == PROTOCOL_BINARY_RESPONSE_SUCCESS) {
        ...
        if (nkey == 0) {
            if (lcb_lookup_server_with_command(root, PROTOCOL_BINARY_CMD_STAT,
                ...
                TRACE_STATS_END(res->response.opaque, packet->vbucket,
                                res->response.opcode, rc);
                root->callbacks.stat(root, packet->cookie, rc, &resp);
            }
            return;
        }
        ...
        setup_lcb_server_stat_resp_t(&resp, server->authority, key,
                                     nkey, value, nvalue);
        TRACE_STATS_PROGRESS(res->response.opaque, packet->vbucket,
                             res->response.opcode, rc, &resp);
        root->callbacks.stat(root, packet->cookie, rc, &resp);
    } else {
        ...
        if (lcb_lookup_server_with_command(root, PROTOCOL_BINARY_CMD_STAT,
            ...
            root->callbacks.stat(root, packet->cookie, LCB_SUCCESS, &resp);
        }
    }
}
                                       ...
                                       protocol_binary_response_header *res)
{
    ...
    TRACE_VERBOSITY_END(res->response.opaque, packet->vbucket,
                        res->response.opcode, rc, &resp);
    root->callbacks.verbosity(root, packet->cookie, rc, &resp);
    ...
    if (lcb_lookup_server_with_command(root, PROTOCOL_BINARY_CMD_VERBOSITY,
        ...
        root->callbacks.verbosity(root, packet->cookie, LCB_SUCCESS, &resp);
    }
}
                                     ...
                                     protocol_binary_response_header *res)
{
    ...
    TRACE_VERSIONS_PROGRESS(res->response.opaque, packet->vbucket,
                            res->response.opcode, rc, &resp);

    root->callbacks.version(root, packet->cookie, rc, &resp);

    if (lcb_lookup_server_with_command(root, PROTOCOL_BINARY_CMD_VERSION,
                                       res->response.opaque, server) < 0) {
        memset(&resp, 0, sizeof(resp));
        TRACE_VERSIONS_END(res->response.opaque, packet->vbucket,
                           res->response.opcode, LCB_SUCCESS);
        root->callbacks.version(root, packet->cookie, LCB_SUCCESS, &resp);
    }
                                   ...
                                   protocol_binary_response_header *res)
{
    ...
    TRACE_TOUCH_END(res->response.opaque, packet->vbucket,
                    res->response.opcode, rc, &resp);
    instance->callbacks.touch(instance, packet->cookie, rc, &resp);
}
                                   ...
                                   protocol_binary_response_header *res)
{
    ...
    TRACE_FLUSH_PROGRESS(res->response.opaque, packet->vbucket,
                         res->response.opcode, rc, &resp);
    root->callbacks.flush(root, packet->cookie, rc, &resp);

    if (lcb_lookup_server_with_command(root, PROTOCOL_BINARY_CMD_FLUSH,
                                       res->response.opaque, server) < 0) {
        setup_lcb_flush_resp_t(&resp, NULL);
        TRACE_FLUSH_END(res->response.opaque, packet->vbucket,
                        res->response.opcode, LCB_SUCCESS);
        root->callbacks.flush(root, packet->cookie, LCB_SUCCESS, &resp);
    }
}
                                    ...
                                    protocol_binary_response_header *res)
{
    ...
    TRACE_UNLOCK_END(res->response.opaque, packet->vbucket, rc, &resp);
    root->callbacks.unlock(root, packet->cookie, rc, &resp);
}
...
int lcb_dispatch_response(lcb_server_t serv,
                          lcb_packet_t pkt,
                          protocol_binary_response_header *res)
{
    switch (res->response.opcode) {
    case PROTOCOL_BINARY_CMD_FLUSH:
        flush_response_handler(serv, pkt, res);
        break;
    ...
    case CMD_GET_LOCKED:
        getq_response_handler(serv, pkt, res);
        break;
    case CMD_GET_REPLICA:
        get_replica_response_handler(serv, pkt, res);
        break;
    case CMD_UNLOCK_KEY:
        unlock_response_handler(serv, pkt, res);
        break;
    case PROTOCOL_BINARY_CMD_DELETE:
        delete_response_handler(serv, pkt, res);
        break;
    case PROTOCOL_BINARY_CMD_ADD:
    case PROTOCOL_BINARY_CMD_REPLACE:
    case PROTOCOL_BINARY_CMD_SET:
    case PROTOCOL_BINARY_CMD_APPEND:
    case PROTOCOL_BINARY_CMD_PREPEND:
        store_response_handler(serv, pkt, res);
        break;
    case PROTOCOL_BINARY_CMD_INCREMENT:
    case PROTOCOL_BINARY_CMD_DECREMENT:
        arithmetic_response_handler(serv, pkt, res);
        break;
    case PROTOCOL_BINARY_CMD_SASL_AUTH:
        sasl_auth_response_handler(serv, pkt, res);
        break;
    case PROTOCOL_BINARY_CMD_SASL_STEP:
        sasl_step_response_handler(serv, pkt, res);
        break;
    case PROTOCOL_BINARY_CMD_TOUCH:
        touch_response_handler(serv, pkt, res);
        break;
    case PROTOCOL_BINARY_CMD_STAT:
        stat_response_handler(serv, pkt, res);
        break;
    case PROTOCOL_BINARY_CMD_VERSION:
        version_response_handler(serv, pkt, res);
        break;
    case PROTOCOL_BINARY_CMD_VERBOSITY:
        verbosity_response_handler(serv, pkt, res);
        break;
    case CMD_OBSERVE:
        observe_response_handler(serv, pkt, res);
        break;
